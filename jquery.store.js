(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			this.paypalCurrency = "RUPEES"; // PayPal's currency code
			...
			if( this.$formCart.length ) {
				...
				if( items.length == 0 ) {
					...
					for( var i = 0; i < items.length; ++i ) {
						...
						var qty = item.qty;
						var html = "<tr style='text-align:center;border: 20px;background-color: #70cfb7;'><td class='pname'>" + product + "</td>" + "<td class='pqty'><input type='number' value='" + qty + "' class='qty'/></td>";
